...
$tit_name=$_GET['title_name']; 
$tit_name=mysql_real_escape_string($tit_name);
if ($tit_name == "")
{
echo '<h3> No match found. Please provide the publisher name </h3>';
}
else
{

  //run a select query 
  $select_query = "select price,img_name,title,isbn,book_desc from book where title like '%$tit_name%'"; 
  $result = mysql_query($select_query) or die (mysql_error()); 
   
  $num_rows = mysql_num_rows($result);
  
   echo'<div class="panel-heading panel-heading-green">';
	echo'<h3 class="panel-title"> <b>Title Matches:  </b></h3>';
	echo'</div>';
	echo '<br/><br/>';
	if ($num_rows == 0)
