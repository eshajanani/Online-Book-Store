...
{
  //run a select query 
  $select_query = "select price,img_name,title,isbn,book_desc from book where publisher like '%$publisher%'"; 
  $result = mysql_query($select_query) or die (mysql_error()); 
   
  $num_rows = mysql_num_rows($result);
  
   echo'<div class="panel-heading panel-heading-green">';
	echo'<h3 class="panel-title"> <b>Publisher Matches:  </b></h3>';
	echo'</div>';
	echo '<br/><br/>';
	if ($num_rows == 0)
